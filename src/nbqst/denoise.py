...
def _simplex_project(eigenvalues, xp):
    ...
    indices = xp.arange(1, d + 1, dtype=ordered.dtype, device=device_of(eigenvalues))
    candidates = cssv / indices
    valid = ordered - candidates > 0
    negative_inf = xp.asarray(float("-inf"), dtype=ordered.dtype, device=device_of(eigenvalues))
    theta = xp.max(xp.where(valid, candidates, negative_inf))
    return xp.maximum(eigenvalues - theta, xp.asarray(0.0, dtype=eigenvalues.dtype, device=device_of(eigenvalues)))
...
def low_rank_projection(matrix, rank: int, *, shrinkage: float = 0.0):
    ...
    positions = xp.arange(dim, device=device_of(values))
    keep = positions >= dim - rank
    zero = xp.asarray(0.0, dtype=values.dtype, device=device_of(values))
    ...
    safe_total = xp.maximum(total, xp.asarray(1e-15, dtype=values.dtype, device=device_of(values)))
...
def depolarizing_shrinkage(matrix, alpha: float):
    ...
    xp = array_namespace(matrix)
    dim = matrix.shape[-1]
    identity = xp.eye(dim, dtype=complex_dtype(xp), device=device_of(matrix)) / dim
